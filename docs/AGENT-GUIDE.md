...
5. **play / export** — `auto-ascii play <clip | composition>` and `compose play
   <name>` are interactive (no `--json`; no re-encode): `q` quits, `v` controls, `/`
   glyph codec, `s` saves dials + codec as `<clip>.player.toml`. A bare name is the
   library clip first (a `.toml` path forces a composition); `compose export <name>
   [-o path]` flattens one into `exports/<name>.ascii`.
