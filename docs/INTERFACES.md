...
pub mod codec {
  pub trait GlyphCodec {
    ...
    const PAD: Cell = Cell::BLANK;  // letterbox pads + composition gaps
    fn cell(&CellInputs, lut: &[u8;256], &PaletteSet, &ComposeParams,
            &mut CellState) -> (Cell, u8 /* layer id */);  // never allocates
  }
  pub enum Codec { Pixels /*default*/, Letters, Ascii }  // generated by registry!
  impl Codec {
    pub const ALL: [Codec; 3];            // `/` cycle order, default first
    pub fn name(self) -> &'static str;    // "pixels" | "letters" | "ascii"
    pub fn pad(self) -> Cell;             // GlyphCodec::PAD
    pub fn from_name(&str) -> Option<Codec>;
    pub fn next(self) -> Codec;           // wraps
    pub fn names(sep: &str) -> String;    // "pixels, letters, ascii" — help/usage
  }
  ...
  pub mod ascii {                         // printable ASCII, no background
    pub struct Ascii;                     // PAD and every cell: DEFAULT_BG
    pub const ASCII_RAMP, ASCII_TOP, ASCII_BOTTOM: &[char]; // 18 each
    pub const ASCII_INK: &[u8];           // Q8 JetBrains Mono coverage
    pub fn ascii_glyphs() -> Vec<char>;   // the repertoire, all 0x20..=0x7E
  }
}
// Adding a codec: one module implementing GlyphCodec + ONE line in the
// registry! list in codec/mod.rs (`Variant => module::Type`), which declares
// the module and generates the variant, its ALL entry, name() and the
// compose_frame_codec arm. The bin's --codec help and headless-dump's usage
// are built from Codec::names, so neither needs an edit.
...
27. **M6 landed** (key-hints agent; PLAN-M6-M8 §1 — "the player should show,
    ...
    tint and no background colour at all. `Cell::attrs` gained its first
    bit, `attrs::DEFAULT_BG`; `FramePainter` emits SGR 49 for such a cell
    instead of a 38/48 background, so the terminal's own background shows
    ...
    just before `RESTORE_SEQ` on every restore path, exactly once.
    ...
    per video. Terminals without OSC 11 ignore both sequences; one that
    ...
    reaches `@` at 216 (was 240), bent up below mid-gray instead of down
    above it. Measured with a Ghostty-approximating renderer on black, the
    face's contrast above black is 0.99× letters on the Architect (was
    0.38× on slate), 1.8× on Terminator, 1.3× on Dune and 0.66× on
    Interstellar; mean luminance stays 0.43–0.98× letters on faces and
    0.41–0.78× on frames, because a glyph inks at most 0.28 of its cell.
    Glyph switches 0.99× pixels. The dial readout marks `(floor)`,
    `(default)` and `(max)` (`Dial::readout`). Tests: `pty_restore.rs`
    (`backdrop_is_set_on_entry_and_reset_on_drop`,
    `backdrop_is_reset_on_sigterm_and_panic`, and no OSC 11/111 without
    it), `codec::ascii::tests` colour bands rewritten for the new curve,
