...
the font size. The terminals that let a program do it all need something
the viewer has to set up first: a config flag, a Lua hook, or an OS
permission prompt. The one sequence that does it without setup (xterm's
OSC 50) is xterm-only. An in-app arrow zoom would work in a few terminals,
after setup, and in all the others it would do nothing and give no sign
why. So the player tells the viewer about the terminal's own zoom keys
instead.
...
| xterm | Ctrl+right-click font menu | Yes: `OSC 50 ; #+1 BEL` / `#-1` steps through the font menu [6] | the `allowFontOps` resource, which I think defaults to on (not verified); xterm only (urxvt has its own OSC 50) |
...
| Alacritty | Ctrl/Cmd +/- | Maybe: `alacritty msg config font.size=…` over the IPC socket named by `ALACRITTY_SOCKET` (from memory, not checked) | the `alacritty` binary on PATH, IPC on, Unix only |
...
as it changes. From 240 columns the overlay text is drawn in 3x5 block
letters (block tiers only), so it is still readable after zooming out.
