...
//! downscales it to the viewport grid through slpy-core's own [`Resampler`]
...
//! `slpy_core::compose::layer` ids filled by `compose_frame_masked`;
...
use slpy_core::compose::layer;
use slpy_core::{Grid, Resampler, Viewport};
...
/// runs through slpy-core's [`Resampler`] — the identical box-average the
...
/// render-metadata `LayerMask` (`Grid<u8>` of `slpy_core::compose::layer`
