//! `slpy-eval` — quantitative metrics for the eval harness (PLAN §6, M2).
//!
...
//! `sleepy-factory eval` (PLAN §5, M2 item B) — this crate deliberately does
//! no I/O beyond serde.
//!
...
//! source Y  ── slpy-core Resampler (same box-average) ──▶ GrayImage
//!                       └────────── ssim ──────────┘
//! ```
//!
...
//! synthetic SLPY assets that the committed goldens and the resize fuzzer
//! run against — pure integer plane generators through `SlpyWriter`, all in
