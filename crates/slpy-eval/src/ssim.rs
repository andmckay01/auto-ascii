...
/// (slpy-core §3.3 — same box-average semantics the player uses), then SSIM.
///
...
pub fn downscale_ssim(rendered: &GrayImage, src_luma: &[u8], src_w: u16, src_h: u16) -> f64 {
    ...
    let mut rs = slpy_core::Resampler::build(src_w, src_h, rendered.w(), rendered.h());
...
mod tests {
    ...
    fn downscale_ssim_prefers_faithful_render() {
        ...
        let mut rs = slpy_core::Resampler::build(480, 270, 96, 54);
