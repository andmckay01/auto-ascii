...
//!   probe cache enabled, rooted at `$ASCII_HARNESS_CACHE_DIR` (M5 cache ×
...
/// rooted at `$ASCII_HARNESS_CACHE_DIR` (test-owned temp dir — never the
...
fn main() {
    ...
    match mode.as_str() {
        ...
        // $ASCII_HARNESS_CACHE_DIR) — the M5 cache×quirk regressions.
