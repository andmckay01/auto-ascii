...
| `conservative.toml` | none — the committed auto-ascii-eval `CONSERVATIVE_COVERAGE` constants (DejaVu-derived, M2) in table form | ASCII repertoire only (the probe-default posture) |
...
`auto-ascii-core` at build time — `FontTable::builtin` — and pinned by
`auto-ascii-core` unit tests.)
