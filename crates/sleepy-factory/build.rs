...
//!
//! `sleepy-factory eval` caches built corpus assets keyed by
...
//! this crate's `src/` and slpy-format's `src/` (the two crates whose code
//! determines asset bytes — extract/shots/lut/build stages and the SLPY
...
fn main() {
    ...
    let dirs = [manifest.join("src"), manifest.join("../slpy-format/src")];

    let mut files: Vec<(String, PathBuf)> = Vec::new();
    for (tag, dir) in ["sleepy-factory", "slpy-format"].iter().zip(&dirs) {
        let start = files.len();
    ...
    println!("cargo:rustc-env=SLPY_PIPELINE_FINGERPRINT={h:016x}");
}
