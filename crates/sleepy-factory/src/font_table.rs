//! `sleepy-factory font-table` — offline glyph ink-coverage table generator
//! (PLAN §3.4, M5 item B).
//!
//! Rasterizes **every glyph the 8 shipped palettes can emit** (enumerated
//! from the palette data via `slpy_core::palette::all_palette_glyphs` — never
...
//! `crates/slpy-core/fonts/README.md` documents the regeneration commands.
...
pub struct FontTableArgs {
    ...
    /// constants from slpy-eval) in the same format instead of rasterizing.
...
fn emit(
    ...
    let _ = writeln!(
        out,
        "# Glyph ink-coverage table (PLAN \u{a7}3.4) \u{2014} generated by `sleepy-factory \
         font-table`.\n\
         # DO NOT EDIT: regenerate with the command in crates/slpy-core/fonts/README.md.\n\
...
pub fn generate_from_font(
    ...
    for ch in slpy_core::palette::all_palette_glyphs() {
...
/// The conservative default in table form: the committed slpy-eval DejaVu
...
pub fn generate_conservative(name: &str) -> (String, Vec<String>) {
    let table = slpy_eval::CoverageTable::conservative();
    let rows: Vec<GlyphRow> = slpy_core::palette::all_palette_glyphs()
        .into_iter()
...
mod tests {
    ...
    /// The emitted table round-trips through the slpy-core parser and covers
    /// the full palette enumeration.
    #[test]
    fn table_roundtrips_through_core_parser() {
        let (toml, _) = generate_conservative("conservative");
        let t = slpy_core::FontTable::parse(&toml).unwrap();
