...
//! assets cached by `(input sha, params sha)`) and the determinism-guard
//! fingerprint. Hand-rolled (~70 lines) rather than a new dependency: the
//! factory needs exactly "hash some bytes", nothing keyed or streaming.
...
pub fn sha256(data: &[u8]) -> [u8; 32] {
    ...
    debug_assert!(msg.len().is_multiple_of(64));
