//! The 64-byte fixed SLPY header (PLAN §4). Layout is frozen — M1 (full
//! format) reuses these exact offsets. All integers little-endian.
//!
//! ```text
//!  0  magic "SLPY" 4B          4  version_major u16 (reader rejects if > supported)
//!  6  version_minor u16 (additive only)   8  header_size u32 (=64)
//! 12  flags u32 (bit0 index present, bit1 CRCs present)
//! 16  fps_num/fps_den u16/u16  20  base_w,base_h u16/u16 (480,270)
//! 24  aspect_num/den u16/u16   28  frame_count u32
//! 32  plane_count u8           33  codec u8 (0=raw 1=lz4 2=zstd)
//! 34  filter u8 (0=intra 1=temporal-delta)   35  keyframe_ivl u8 (=60)
//! 36  plane_ids[8] u8×8 (registry: 1=Y 2=E 3=Ex 4=Ey 5=H 6=C ...)
//! 44  index_offset u64 (patched at close)    52  meta_offset u64   60 reserved u32
//! ```

use crate::error::{Result, SlpyError};

pub const MAGIC: [u8; 4] = *b"SLPY";
pub const HEADER_SIZE: u32 = 64;
pub const VERSION_MAJOR: u16 = 1;
pub const VERSION_MINOR: u16 = 0;
/// Base analysis resolution (PLAN §4: planes stored at 480×270).
pub const BASE_W: u16 = 480;
/// See [`BASE_W`].
pub const BASE_H: u16 = 270;

/// Plane-ID registry (PLAN §4). Unknown IDs are skippable by construction —
/// a future motion plane is a new ID, not a version bump.
pub mod plane_id {
    /// L\* luma, base res u8. The only plane at M0 (SLPY-lite, PLAN §7).
    pub const Y: u8 = 1;
    /// Edge magnitude, smoothed, unthinned (M3).
    pub const E: u8 = 2;
    /// Doubled-angle `mag·cos 2θ`, bias-128 (M3).
    pub const EX: u8 = 3;
    /// Doubled-angle `mag·sin 2θ`, bias-128 (M3).
    pub const EY: u8 = 4;
    /// Highlight/deep-shadow flags (M3).
    pub const H: u8 = 5;
    /// Chroma RGB565 at half res 240×135 (M1+).
    pub const C: u8 = 6;
}

/// `codec` field values (PLAN §4).
pub mod codec {
    pub const RAW: u8 = 0;
    pub const LZ4: u8 = 1;
    pub const ZSTD: u8 = 2;
}

/// `filter` field values (PLAN §4). M0 writes INTRA only; TEMPORAL_DELTA at M1.
pub mod filter {
    pub const INTRA: u8 = 0;
    pub const TEMPORAL_DELTA: u8 = 1;
}

/// Header `flags` bits (PLAN §4).
pub mod header_flags {
    /// bit0: FIDX chunk present.
    pub const INDEX_PRESENT: u32 = 1;
    /// bit1: chunks carry trailing CRC32s.
    pub const CRCS_PRESENT: u32 = 1 << 1;
}

/// Parsed/parseable form of the 64-byte header (PLAN §4). `magic`,
/// `header_size` and the reserved u32 are implicit — enforced/emitted by
/// [`SlpyHeader::from_bytes`]/[`SlpyHeader::to_bytes`].
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SlpyHeader {
    pub version_major: u16,
    pub version_minor: u16,
    pub flags: u32,
    pub fps_num: u16,
    pub fps_den: u16,
    pub base_w: u16,
    pub base_h: u16,
    pub aspect_num: u16,
    pub aspect_den: u16,
    pub frame_count: u32,
    pub plane_count: u8,
    pub codec: u8,
    pub filter: u8,
    pub keyframe_ivl: u8,
    /// First `plane_count` entries meaningful; rest zero.
    pub plane_ids: [u8; 8],
    /// Byte offset of the FIDX chunk header; patched at close (PLAN §4).
    pub index_offset: u64,
    /// Byte offset of the META chunk header.
    pub meta_offset: u64,
}

impl SlpyHeader {
    /// Serialize to the exact 64-byte layout above (little-endian,
    /// reserved u32 = 0). Byte-deterministic — golden-tested (PLAN §4/§6).
    pub fn to_bytes(&self) -> [u8; 64] {
        let mut b = [0u8; 64];
        b[0..4].copy_from_slice(&MAGIC);
        b[4..6].copy_from_slice(&self.version_major.to_le_bytes());
        b[6..8].copy_from_slice(&self.version_minor.to_le_bytes());
        b[8..12].copy_from_slice(&HEADER_SIZE.to_le_bytes());
        b[12..16].copy_from_slice(&self.flags.to_le_bytes());
        b[16..18].copy_from_slice(&self.fps_num.to_le_bytes());
        b[18..20].copy_from_slice(&self.fps_den.to_le_bytes());
        b[20..22].copy_from_slice(&self.base_w.to_le_bytes());
        b[22..24].copy_from_slice(&self.base_h.to_le_bytes());
        b[24..26].copy_from_slice(&self.aspect_num.to_le_bytes());
        b[26..28].copy_from_slice(&self.aspect_den.to_le_bytes());
        b[28..32].copy_from_slice(&self.frame_count.to_le_bytes());
        b[32] = self.plane_count;
        b[33] = self.codec;
        b[34] = self.filter;
        b[35] = self.keyframe_ivl;
        b[36..44].copy_from_slice(&self.plane_ids);
        b[44..52].copy_from_slice(&self.index_offset.to_le_bytes());
        b[52..60].copy_from_slice(&self.meta_offset.to_le_bytes());
        // 60..64 reserved = 0
        b
    }

    /// Parse and validate: magic, `header_size == 64`, and
    /// `version_major <= VERSION_MAJOR` (PLAN §4: reader rejects if greater;
    /// minor versions are additive and never rejected).
    pub fn from_bytes(b: &[u8; 64]) -> Result<SlpyHeader> {
        let le16 = |o: usize| u16::from_le_bytes([b[o], b[o + 1]]);
        let le32 = |o: usize| u32::from_le_bytes([b[o], b[o + 1], b[o + 2], b[o + 3]]);
        let le64 = |o: usize| {
            u64::from_le_bytes([b[o], b[o + 1], b[o + 2], b[o + 3], b[o + 4], b[o + 5], b[o + 6], b[o + 7]])
        };

        if b[0..4] != MAGIC {
            return Err(SlpyError::BadMagic);
        }
        let version_major = le16(4);
        if version_major > VERSION_MAJOR {
            return Err(SlpyError::UnsupportedVersion { found: version_major, supported: VERSION_MAJOR });
        }
        if le32(8) != HEADER_SIZE {
            return Err(SlpyError::Corrupt("header_size != 64"));
        }
        let mut plane_ids = [0u8; 8];
        plane_ids.copy_from_slice(&b[36..44]);
        Ok(SlpyHeader {
            version_major,
            version_minor: le16(6),
            flags: le32(12),
            fps_num: le16(16),
            fps_den: le16(18),
            base_w: le16(20),
            base_h: le16(22),
            aspect_num: le16(24),
            aspect_den: le16(26),
            frame_count: le32(28),
            plane_count: b[32],
            codec: b[33],
            filter: b[34],
            keyframe_ivl: b[35],
            plane_ids,
            index_offset: le64(44),
            meta_offset: le64(52),
        })
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn sample() -> SlpyHeader {
        SlpyHeader {
            version_major: 1,
            version_minor: 0,
            flags: header_flags::INDEX_PRESENT | header_flags::CRCS_PRESENT,
            fps_num: 30,
            fps_den: 1,
            base_w: BASE_W,
            base_h: BASE_H,
            aspect_num: 16,
            aspect_den: 9,
            frame_count: 900,
            plane_count: 1,
            codec: codec::ZSTD,
            filter: filter::INTRA,
            keyframe_ivl: 60,
            plane_ids: [plane_id::Y, 0, 0, 0, 0, 0, 0, 0],
            index_offset: 0xDEAD_BEEF,
            meta_offset: 64,
        }
    }

    #[test]
    fn layout_offsets_frozen() {
        let b = sample().to_bytes();
        assert_eq!(&b[0..4], b"SLPY");
        assert_eq!(u32::from_le_bytes(b[8..12].try_into().unwrap()), 64); // header_size @8
        assert_eq!(u16::from_le_bytes(b[16..18].try_into().unwrap()), 30); // fps_num @16
        assert_eq!(u16::from_le_bytes(b[20..22].try_into().unwrap()), 480); // base_w @20
        assert_eq!(u32::from_le_bytes(b[28..32].try_into().unwrap()), 900); // frame_count @28
        assert_eq!(b[32], 1); // plane_count
        assert_eq!(b[33], codec::ZSTD);
        assert_eq!(b[36], plane_id::Y); // plane_ids @36
        assert_eq!(u64::from_le_bytes(b[44..52].try_into().unwrap()), 0xDEAD_BEEF); // index_offset @44
        assert_eq!(&b[60..64], &[0, 0, 0, 0]); // reserved
    }

    #[test]
    fn roundtrip() {
        let h = sample();
        assert_eq!(SlpyHeader::from_bytes(&h.to_bytes()).unwrap(), h);
    }

    #[test]
    fn rejects_bad_magic_and_future_major() {
        let mut b = sample().to_bytes();
        b[0] = b'X';
        assert!(matches!(SlpyHeader::from_bytes(&b), Err(SlpyError::BadMagic)));

        let mut b = sample().to_bytes();
        b[4..6].copy_from_slice(&2u16.to_le_bytes());
        assert!(matches!(
            SlpyHeader::from_bytes(&b),
            Err(SlpyError::UnsupportedVersion { found: 2, .. })
        ));
    }
}
