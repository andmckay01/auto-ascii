...
use slpy_format::{
    CHUNK_HEADER_SIZE, ChunkHeader, Meta, PlaneRef, SlpyError, SlpyReader, SlpyWriter,
...
fn test_meta() -> Meta {
    Meta {
        factory_version: "slpy-format-test-0.1.0".to_string(),
...
fn encode_default() -> Vec<u8> {
    let writer =
        SlpyWriter::new(Cursor::new(Vec::new()), WriterOptions::default(), &test_meta()).unwrap();
...
fn corrupt_fram_payload_fails_crc_verify() {
    ...
    match reader.verify() {
        Err(SlpyError::CrcMismatch { tag }) => assert_eq!(tag, TAG_FRAM),
...
fn corrupt_meta_payload_fails_crc_verify() {
    ...
    let reader = SlpyReader::open(&bytes).unwrap();
    match reader.verify() {
        Err(SlpyError::CrcMismatch { tag }) => assert_eq!(tag, TAG_META),
...
fn missing_trlr_is_truncated() {
    ...
    // Cut the whole TRLR chunk (header + "SLPY_END" + crc).
    let cut = &bytes[..trlr_off];
    assert!(matches!(SlpyReader::open(cut), Err(SlpyError::Truncated)));
}
...
fn mid_chunk_truncation_is_truncated() {
    let bytes = golden_bytes();
    assert!(matches!(
        SlpyReader::open(&bytes[..bytes.len() - 3]),
        Err(SlpyError::Truncated)
    ));
    // Also: cut inside the FRAM stream (before FIDX).
    let (fidx_off, _) = find_chunk(bytes, TAG_FIDX);
    assert!(matches!(
        SlpyReader::open(&bytes[..fidx_off + 5]),
        Err(SlpyError::Truncated)
    ));
}
...
fn unknown_required_chunk_is_rejected() {
    ...
    match SlpyReader::open(&bytes) {
        Err(SlpyError::UnknownRequiredChunk(tag)) => assert_eq!(&tag, b"XREQ"),
...
fn unknown_optional_chunk_is_skipped() {
    ...
    assert!(matches!(reader.meta(), Err(SlpyError::Corrupt(_))));
}
...
fn future_major_version_is_rejected() {
    ...
    assert!(matches!(
        SlpyReader::open(&bytes),
        Err(SlpyError::UnsupportedVersion { found: 2, supported: 1 })
    ));
}
...
fn bad_frame_index_and_bad_plane_id() {
    let mut reader = SlpyReader::open(golden_bytes()).unwrap();
    let mut dst = vec![0u8; W * H];
    assert!(matches!(
        reader.decode_plane_into(FRAMES, plane_id::Y, &mut dst),
        Err(SlpyError::BadFrameIndex(f)) if f == FRAMES
    ));
    assert!(matches!(
        reader.decode_plane_into(0, plane_id::E, &mut dst),
        Err(SlpyError::BadPlaneId(id)) if id == plane_id::E
    ));
}
...
fn writer_rejects_mismatched_planes() {
    ...
    writer.write_frame(&[PlaneRef { id: plane_id::Y, data: &plane }]).unwrap();
    let bytes = writer.finish().unwrap().into_inner();
    assert_eq!(SlpyReader::open(&bytes).unwrap().frame_count(), 1);
}
...
fn writer_rejects_bad_options() {
    ...
    for opts in cases {
        assert!(
            SlpyWriter::new(Cursor::new(Vec::new()), opts.clone(), &test_meta()).is_err(),
