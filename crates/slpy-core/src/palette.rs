//! The 8 shipped palettes (PLAN §3.4) as data, plus `PaletteSet` selection.
//!
//! Selection is keyed **charset tier × layer role** (PLAN §1b): density (cell
//! count) selects ramp *length within* a config — the coarse/fine ASCII split —
//! and color depth selects an *effective ramp-length cap* (truecolor shorter,
//! mono longest) instead of duplicating ramp data.
//!
//! `slpy-core` stays terminal-free: [`GlyphTier`] and [`ColorDepth`] are
//! Caps-shaped input enums the player derives from `slpy-term`'s probe result
...
pub enum GlyphTier {
    ...
    UnicodeBlocks,
    /// Braille U+2800–28FF *verified* present (PLAN §3.4 palette 7 gate).
    BrailleVerified,
}

/// Color depth — mirrors `slpy-term::ColorTier` variants without the
/// dependency. Drives the per-tier ramp-length caps (PLAN §1b: truecolor gets
/// shorter effective ramps because color carries luminance; mono the longest).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum ColorDepth {
    True,
    C256,
...
/// Density band (PLAN §3.4: coarse below 70 viewport cols).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
...
/// Layer role — the second axis of the §1b selection key. `PaletteSet` holds
/// one choice per role; the enum names them for docs/tests.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum LayerRole {
    Base,
    Edge,
...
/// Vertical sub-cell position from the Vc×2Vr luma pair (PLAN §3.3/§3.5).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum SubPos {
    Top = 0,
    Mid = 1,
...
/// plus junction glyphs for conflicting bins (PLAN §3.4 palettes 3 and 6).
#[derive(Debug, PartialEq, Eq)]
pub struct EdgeLut {
    pub by_class: [[char; 3]; 4],
    /// Emitted when orientation bins conflict inside the cell (coherence in
    /// the junction band, §3.5).
    pub junction: char,
    /// Junction at strong edge magnitude (≥ `ComposeParams::edge_strong`).
    pub junction_strong: char,
}

/// Palette 3 — `ascii/edge` (PLAN §3.4): `- / | \` + `_ = + #` (junction).
///
/// Deviations forced by ASCII: there is no overline glyph, so the horizontal
/// top-subposition slot uses `=` (the closest raised-ink horizontal in the
/// table's glyph set); `+` is the junction, `#` the strong junction.
...
/// Palette 6 — `unicode/edge` (PLAN §3.4): `─ │ ╱ ╲ ‾ _ ┼` (8-dir + junction).
...
/// Palette 4 — `ascii/highlight` (PLAN §3.4): `" .+*"`. Shared by all tiers —
/// the table defines no separate unicode highlight ramp.
pub const ASCII_HIGHLIGHT: &[char] = &[' ', '.', '+', '*'];

/// Palette 5 (base half) — `unicode/base` (PLAN §3.4): `" ·░▒▓█"`.
pub const UNICODE_BASE: &[char] = &[' ', '·', '░', '▒', '▓', '█'];

/// Palette 5 (quadrant half) — `▖▘▝▗▀▄▌▐` from the 2×2 pattern (PLAN §3.4).
///
/// v1 approximation (deliberate, PLAN 80/20): the runtime samples luma at
/// Vc×2Vr only, so the 2×2 pattern is *reconstructed* from the vertical pair
/// plus the dominant orientation — diagonal orientations pick corner quadrants
/// via [`quadrant_for`]; `▀▄` are the half-block path; `▌▐` are unreachable
/// until a full 2Vc×2Vr sampling upgrade (kept here for §3.4 table exactness —
/// do NOT build a second resample path for them now).
...
pub const BRAILLE_EDGE: BrailleLut = BrailleLut {
    by_class: [
        [0x09, 0x36, 0xC0], // H: row 1 / rows 2–3 / row 4
        [0xA3, 0xA3, 0xA3], // DiagDown: c1 top pair + c2 bottom pair
        [0x47, 0x47, 0x47], // V: left column
        [0x5C, 0x5C, 0x5C], // DiagUp: c2 top pair + c1 bottom pair
    ],
    junction: 0x57, // left column + middle row cross, 5 dots
};

/// Braille char for a dot mask: U+2800 + mask (always a valid scalar).
#[inline]
pub fn braille_glyph(mask: u8) -> char {
    // SAFETY-free: 0x2800..=0x28FF are all valid scalar values.
...
/// serves every color tier, `len` just quantizes coarser (PLAN §1b "encode as
/// ramp-length caps, not duplicated data"). Indices 0..len spread over the
/// full glyph range with exact endpoints (0 → first, len−1 → last).
...
/// configuration — the compositor's §3.5 lookup surface.
...
pub struct PaletteSet {
    ...
    /// Half-block `▀▄` (fg,bg) pairs available (unicode tiers, §3.5).
    pub halfblock: bool,
...
/// Select the palette configuration (PLAN §1b key: charset tier × layer role;
...
pub fn select_palettes(tier: GlyphTier, color: ColorDepth, viewport_cols: u16) -> PaletteSet {
    ...
    let cap = match color {
        ...
        // §1b: mono/16-color tiers get the longest ramps — no cap.
        ColorDepth::C16 | ColorDepth::Mono => u8::MAX,
    };
    let base_glyphs: &'static [char] = match color {
        // Palette 8 is keyed on the terminal class (16-color / no-color /
        // Linux console), not the charset tier — CP437-safe by construction.
...
mod tests {
    ...
    /// PLAN §3.4 table exactness — ramp strings verbatim.
    #[test]
    fn palette_table_exact() {
        assert_eq!(s(ASCII_BASE_COARSE), " .:-=+*#%@"); // 1
        assert_eq!(s(ASCII_BASE_FINE), " .,:;i1tfLCG08@"); // 2
        assert_eq!(s(ASCII_HIGHLIGHT), " .+*"); // 4
    ...
    /// Palette 7: braille stays in U+2800–28FF and is never solid (≤ 5 dots).
    #[test]
    ...
    fn ramp_view_caps_and_spreads() {
        ...
        assert_eq!(v.len(), 8);
        assert_eq!(v.glyph(0), ' '); // exact endpoints
        assert_eq!(v.glyph(7), '@');
        let full = RampView::new(ASCII_BASE_FINE, u8::MAX);
        assert_eq!(full.len(), 15);
        assert_eq!(full.glyph(14), '@');
        assert_eq!(full.glyph(3), ':'); // identity when uncapped
    }

    #[test]
    fn selection_matrix() {
        // Ascii + truecolor + coarse: coarse ramp capped to 8, subpos on.
        ...
        // Ascii + 256 + fine: fine ramp capped to 12.
        ...
        // Braille: verified tier AND fine density only (PLAN §3.4 row 7).
    ...
    fn subpos_classification() {
        ...
        assert_eq!(subpos(128, 128, 0), SubPos::Mid); // delta 0 never decisive
    }
}
