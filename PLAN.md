...
**Engineering posture:** 80/20 throughout; the eval harness lands *before* the quality-critical layer work so every aesthetic decision from then on is measured; the worst case (300×80 in tmux over 20 ms-RTT SSH) shapes the output path, and a local Kitty window is the degenerate easy case.
...
- **M2 — harness online (before quality work).** Metrics JSON, goldens ×3 grids ×3 palettes, 10k fuzz cases with full invariant set, criterion gates, **2 MB/s SimBackend ≥24 fps end-to-end gate**, HTML contact sheets. *Accept:* one command runs the loop <5 min; a deliberate 20% perf regression and a deliberate param regression both fail CI.
...
- **M4 — slow-tier reality.** Governor downshift ladder verified on real links; tmux, SSH-over-WAN, ConPTY. *Accept:* effective ≥24 fps (with drops) over 20 ms-RTT SSH at ≤50 KB/frame average; tmux artifact-free; Windows Terminal plays; `TERM=linux` legible with palette 8.
...
2. **SSH/tmux throughput collapse.** *Mitigation:* quantize-before-diff + elision + hysteresis-as-damage-control; governor with measured `write_ns`; drop-frames pacing is correctness, not failure; throttled SimBackend CI gate from M2; M4 acceptance on a real WAN link. Mono floor ≈8 KB/frame.
